...
public class Sphere implements IShapeColored {
	...
	public HitColoredDistance getHit(Ray ray) {
		...
		if (underSqrt == 0) {
			return new HitColoredDistance(minusPHalf, color);
		}
		...
		if (t1 > 0 && t2 > 0)
			return t1 > t2 ? new HitColoredDistance(t2, color) : new HitColoredDistance(t1, color);

		if (t1 <= 0 && t2 > 0)
			return new HitColoredDistance(t2, color);

		if (t1 > 0 && t2 <= 0)
			return new HitColoredDistance(t1, color);
